...
Drenajes = arcpy.GetParameterAsText(0)
Ruta_Salida = arcpy.GetParameterAsText(1)
Buffer = arcpy.GetParameterAsText(2)

arcpy.env.overwriteOutput = True
arcpy.env.workspace = Drenajes
arcpy.AddMessage("INICIANDO PROCESO")

#1 HACER CAPA DE PUNTOS DE DANGLES. INICIO Y FINAL
Dangles = arcpy.FeatureVerticesToPoints_management(Drenajes,os.path.join(str(Ruta_Salida),'Dangles.shp'), "DANGLE")
PInicioFin = arcpy.FeatureVerticesToPoints_management(Drenajes,os.path.join(str(Ruta_Salida),'PInicioFin.shp'), "BOTH_ENDS")
...
ruta_nueva_capa = os.path.join(Ruta_Salida, "Puntos_Validar_Drenajes.shp")
...
with arcpy.da.SearchCursor(Puntos, ["SHAPE@","ORIG_FID"]) as Scursor:
    for punto in Scursor:
        ...
        Selectp = 0
        Selectp = arcpy.management.SelectLayerByLocation(Drenajes, "INTERSECT", punto[0], search_distance=str(Buffer)+' Meters')
...
arcpy.AddMessage(f"Se han insertado {arcpy.GetCount_management(ruta_nueva_capa)} puntos con drenajes sin conectar")
