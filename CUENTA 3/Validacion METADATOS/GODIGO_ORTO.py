...
if depto == True: 
    ...
    try:
        ...
        if not str(x).startswith('Ortoimagen. Departamento de ') or ('. GSD ' not in x) or (' cm. Año ' not in x):
            error+=1
        else:
            error=0
        if error>0:
...
try: 
    ...
    if ('Ortoimágen compuesto de imágenes ortorectificadas provenientes del sensor ' not in str(x) or 
        'a las cuales se les aplicó un proceso de balance radiométrico y edición de líneas de costura, garantizando la continuidad cromática y geométrica de los elementos. Este producto contiene información de ' not in x or 
        ', departamento de' not in x or ', República de Colombia. Tiene un área de ' not in x or ' hectáreas. Cuenta con un GSD de ' not in x or 
        ', aplicable para cartografía a escala 1:' not in x or ', los insumos fueron capturados el día ' not in x or
        'de ' not in x or
        'de ' not in x
        ):
        error+=1
...
if depto == True: 
    ...
    try:
        ...
        if not str(x).startswith('República de Colombia, Departamento de '): 
            error+=1
        else:
            error=0
        if error>0:
        ...
        arcpy.AddMessage("No se encuentra la etiqueta keyword dentro del esquema XML ")
else:
    error=0
    try:
        x=getdata_from_xml("keyword")
...
if error>0:
    ...
    with arcpy.da.InsertCursor(summary, ['id_validacion', 'etiqueta', 'observacion']) as cursor:
        for row in row_values:
            cursor.insertRow(row)
excel = arcpy.TableToExcel_conversion(summary, os.path.join(folder_output, 'ValidacionMetadato_'+str(tiempo_actual())+'_'+str(os.path.basename(layer)).replace(".","_")+'.xlsx'))
try:
