...
def vectores(shp_vectores, raster_dataset, shp_limite, ruta_salida,escala,margen):
    ...
    # Se calcula la diferencia entre la altura y el valor del raster para cada punto de validación
    arcpy.AddField_management(puntos_validacion, 'DIFERENCIA', 'double')
    arcpy.CalculateField_management(puntos_validacion, 'DIFERENCIA', '(!altura!-!RASTERVALU!)','PYTHON3')
...
def cad(dgn, raster_dataset, shp_limite, ruta_salida, escala, margen):
    ...
    arcpy.conversion.FeatureClassToShapefile([polylinea], ruta_salida)
    arcpy.management.Rename('Polyline_Layer2.shp', 'vectores.shp')
    vectores_piso = os.path.join(str(ruta_salida),'vectores.shp')
